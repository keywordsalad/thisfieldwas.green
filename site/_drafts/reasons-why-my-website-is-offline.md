...
This sudden urge to self host was impulsed by the advent of GitHub Copilot. The concept of it sure spooked the hell out of me. Not the idea that my professional role had become so fungible that I could be replaced by a machine, I feel that is inevitable regardless, but rather that I unknowingly, and in the essence of the freest of products, that I was myself the source of the data for which _corporations_ would be paying untold sums of money to _another corporation_ rather than to _people_ in order to manifest technical means of production; my labor having been willingly consented to subsidize widening profit margins in what felt to me a very real embodiment of late stage capitalism. I was very motivated to get as much of my stuff out of third party services as I could because it was clear, as this also coincided with Apple’s neural hashing of photos, that third parties could not be trusted with my data _even if they were already making money off of it_. I disconnected myself from cloud server web and storage hosting in addition.
...
The computer sat in many pieces in my living room for about a month as I removed dust and prepared to install an array of disk to host my media. HP did not make this case amenable to changes or even provide much in the way of expansion bays, so as a consequence I now have a drive array that is free-floating within the case. No matter though, the drives are each `4TB` and so heavy that they are held in place by friction induced by their own mass and density alone. Later for a short period the computer carried the hostname `gravwell` because of the density these drives gave it.

After I carefully arranged the disks within and had an Arch installation that could be reproducibly bootstrapped, I closed up the case and moved the computer into the bedroom closet where I could plug it directly into the router.
...
My goal with this playbook was to be able to reproduce the configuration of the closet computer on a replacement if it should ever discontinue to work. Happy path would be a matter of a few keystrokes, grabbing a coffee while the playbook ran, and then powering off the computer and plugging the drive array into it. Configuration management is Ansible's primary domain, it does this very well, and I now use it for my current three Linux-based systems.

I have very much and many times nuked and enjoyed rebuilding my Arch Linux-powered _thonkpad_. All changes I make to it are managed through its associated playbook, recently including an exploration of Minecraft launchers I performed after Microsoft seemingly required me to migrate my Mojang/Minecraft account and fried authentication into the game.
...
* A big, _just a bunch of disks_ array providing `16TB` of usable storage, with redundancy provided by [`snapraid`](https://www.snapraid.it/), and unioned with [`mergerfs`](https://github.com/trapexit/mergerfs) so that functionally it looks like one big fat disk. It’s very nice to use.
* `pihole`... I thought this would mean less ads, but it's mostly been more headaches.

I’m using docker containers for `gitea` and `postgres` (which supports `gitea`). `nginx` runs natively and provides routing for each domain I have configured for it.
...
Here is a short, incomplete list of reasons why my website has been offline:
...
* The computer came up before the router and `autossh` stops trying to open the tunnels to the outside network after a period of time because `systemd` flakes.
...
I don’t have a static IP. My ISP’s network infrastructure simply doesn’t allow for it, as it is based on a mesh of wireless signal propagated between antennas installed on top of each house using their service. To get around this, I had to swallow my pride and purchase a cloud server from [`linode`](https://linode.com), I call this my bastion server, and I configured my domains to point to its IP address.

With the bastion server as a static anchor that my closet computer could reach out to, I could then enable my website to come out of the closet:

* I use `autossh` with an unprivileged user to maintain `ssh` tunnels that end at public ports `22`, `80`, and `443` on the closet computer.
* These `ssh` tunnels begin at the bastion, privately bound to its `127.0.0.1` ports `10022`, `10080`, and `10443`.
* An `nginx` streaming server on the bastion is configured to pass all traffic from the bastion's public ports `22`, `80`, and `443` to the `127.0.0.1` ports `10022`, `10080`, and `10443`.
* As the `ssh` tunnels simply pass bits from point `A` to point `B`, to an outside observer it's as if my website is coming from the bastion server, when in fact it's coming out of the closet.

#### It basically looks like this:

```{.txt .nowrap}
|-the world-|  |-bastion-------------------|  |-closet-------|
-------------> | stream--> ssh tunnel-------> | all my stuff
                 *:22      127.0.0.1:10022      *:22
                  :80               :10080       :80
                  :443              :10443       :443
```
...
This is a really fragile architecture, however. If the `ssh` tunnels aren’t up, the sites can’t be served and I can’t even get an error page out as there doesn't seem to be a way to query for whether the streaming server is passing any data through. This setup is really kinda crap like that.

Being what it is, `systemd` on the closet computer will stop `autossh` from retrying connections to the bastion after a certain period and this is a major problem if there’s a hiccup in the home internet connection. Sometimes my internet is out for five minutes, but that’s a tradeoff made when using a cheaper, locally-based ISP over Comcast's cartoon villain bundled contracts. At a minimum I need to use something other than `systemd` to manage my `ssh` tunnels as it hasn’t been reliable _at all_ and probably one of the bigger points of failure because it cascades with any failures the router experiences.

Physically the closet receives too much human traffic for the closet computer or really any hardware to be held there. We live in a century craftsman whose modern infrastructure has been monkey-patched together and I piggybacked on the convenient, pre-existing connections made by the previous homeowners putting hardly a thought into any consequences therein. Now we have a closet acting as a server cabinet instead of a closet. We are of course using it as both, with frequent access throughout the day and cables running everywhere as if a panicked octopus were dropped on the floor and frozen in place.
...
Despite this disdain I'm expressing, third party services are invaluable for their uptime. I severely took this for granted and it’s making me rethink what total cost actually looks like, not only in terms of dollars and nebulous ideals but even simple booleans such as _will the manager be able to see that writeup I referred her to?_ I would absolutely benefit from hosting my website from my bastion server instead, perhaps after I reread `linode`'s service agreements to allay my confounding mind.
